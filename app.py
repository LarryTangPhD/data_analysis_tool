...
from src.utils.ml_helpers import (
    train_classification_model, train_regression_model, perform_clustering,
    create_confusion_matrix, create_feature_importance
)
...
# 导入专业数据洞察模块
from src.utils.profiling_insights import (
    render_ydata_profiling_insights, render_sweetviz_insights, 
    render_comprehensive_insights, render_quick_insights,
    render_data_quality_assessment, check_tool_availability
)

# 导入新的优化模块
from src.utils.data_science_workflow import (
    DataScienceWorkflow, render_workflow_dashboard, 
    create_custom_workflow, validate_data_for_workflow
)
from src.utils.advanced_visualization import render_advanced_visualization_page
from src.utils.performance_optimizer import performance_optimizer
from src.utils.user_experience import (
    user_guide, feedback_system, help_system, 
    progress_tracker, notification_system, render_user_experience_components
)
# 导入高级格式转换模块
from src.utils.advanced_format_converter import render_format_conversion_section
# 导入数据格式转换页面
...
if page == "🎯 模式选择":
    ...
    if uploaded_file is not None:
        try:
            ...
            if uploaded_file.name.endswith('.json'):
                from src.utils.data_processing import validate_json_structure
    ...
    st.markdown('<h2 class="sub-header">📊 数据科学工作流管理</h2>', unsafe_allow_html=True)
    ...
    if not hasattr(st.session_state, 'data') or st.session_state.data is None:
        ...
        if 'workflow' not in st.session_state:
            st.session_state.workflow = DataScienceWorkflow()
        ...
        if workflow_type == "快速分析":
            workflow = create_custom_workflow("快速分析")
        elif workflow_type == "深度分析":
            workflow = create_custom_workflow("深度分析")
        
        # 数据验证
        validation_results = validate_data_for_workflow(data)
        ...
        if not validation_results["is_valid"]:
            ...
            # 显示工作流仪表板
            render_workflow_dashboard(workflow)
            ...
            with col1:
                if st.button("🚀 开始工作流", type="primary"):
                    workflow.start_step("数据加载与验证")
                    st.success("✅ 工作流已开始")
            
            with col2:
                if st.button("⏸️ 暂停工作流"):
                    st.info("工作流已暂停")
            ...
            for i, step in enumerate(workflow.steps):
                ...
                with col3:
                    if not step.completed:
                        if st.button(f"执行 {step.name}", key=f"execute_{i}"):
                            ...
                            with st.spinner(f"正在执行 {step.name}..."):
                                ...
                                # 根据步骤类型执行相应操作
                                if step.name == "数据加载与验证":
                                    metrics = {
                                        "数据行数": len(data),
                                        "数据列数": len(data.columns),
                                        "缺失值数量": data.isnull().sum().sum()
                                    }
                                    workflow.complete_step(step.name, metrics)
                                
                                elif step.name == "探索性数据分析":
                                    metrics = {
                                        "数值型列数": len(data.select_dtypes(include=[np.number]).columns),
                                        "分类型列数": len(data.select_dtypes(include=['object']).columns),
                                        "数据质量评分": calculate_data_quality_score(data)
                                    }
                                    workflow.complete_step(step.name, metrics)
                                
                                else:
                                    workflow.complete_step(step.name)
