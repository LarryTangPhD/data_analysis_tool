...
def process_json_data(data: pd.DataFrame) -> pd.DataFrame:
    """
    处理JSON数据中的复杂对象类型
    
    Args:
        data: 包含复杂对象的DataFrame
    ...
    for col in processed_data.columns:
        if processed_data[col].dtype == 'object':
            # 检查列中是否包含字典或列表
            sample_values = processed_data[col].dropna().head(5)
...
def validate_json_structure(data: pd.DataFrame) -> Dict[str, Any]:
    """
    验证JSON数据结构并提供处理建议
    
    Args:
        data: JSON数据DataFrame
        ...
        if data[col].dtype == 'object':
            ...
            if len(sample_values) > 0:
                ...
                if isinstance(first_value, dict):
                    validation_result["complex_columns"].append({
                        "column": col,
                        "type": "dict",
                        "sample_keys": list(first_value.keys()) if first_value else []
                    })
                    validation_result["suggestions"].append(
                        f"列 '{col}' 包含字典数据，已转换为字符串格式"
                    )
                elif isinstance(first_value, list):
                    validation_result["complex_columns"].append({
                        "column": col,
                        "type": "list",
                        "sample_length": len(first_value) if first_value else 0
                    })
                    validation_result["suggestions"].append(
                        f"列 '{col}' 包含列表数据，已转换为字符串格式"
                    )
    
    if validation_result["complex_columns"]:
        validation_result["warnings"].append(
            f"发现 {len(validation_result['complex_columns'])} 个包含复杂对象的列"
        )
